...
const createNotification = async (req, res) => {
  ...
  const notification = await Notification.create({
    ...
    // رابط بلا https كان يُفشل الإرسال لكل الأجهزة؛ يُحفظ مصحَّحاً فيعمل
    // في الإشعار وفي زر «التفاصيل» داخل الموقع. المسار الداخلي (/...) يبقى كما هو.
    link: String(link || '').trim().startsWith('/')
      ? String(link).trim()
      : pushService.toHttpsUrl(link),
...
// POST /api/notifications/reactivate-subscribers
// استعادة الأجهزة المعطّلة: خلل سابق كان يعطّل كل الأجهزة حين يُرفض الإشعار
// بسبب رابطه. إعادة تفعيل جهاز ميت فعلاً آمنة — أول إرسال قادم يعطّله من جديد
// بتصنيف صحيح (registration-token-not-registered).
const reactivateSubscribers = async (req, res) => {
  const r = await PushSubscriber.updateMany({ isActive: false }, { $set: { isActive: true } });
  const total = await PushSubscriber.countDocuments({ isActive: true });
  console.log(`♻️ ${req.user.username} استعاد ${r.modifiedCount} جهاز مشترك — المجموع الآن ${total}`);
  res.json({ success: true, restored: r.modifiedCount, total, message: `استُعيد ${r.modifiedCount} جهاز — المشتركون الآن ${total}` });
};
