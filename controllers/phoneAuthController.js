...
// ✅ إصلاح: كانت هاتان الدالتان/المتغيّر مستخدمة دون تعريف، فكان كل طلب
// /api/auth/send-otp و /verify-otp ينتهي بخطأ ReferenceError ويُرجع 500.
// التحقق من صيغة الرقم بعد التطبيع (+9627XXXXXXXX)
const isValidPhone = (phone) => /^\+9627\d{8}$/.test(String(phone || ''));

// سجل الإرسال في الذاكرة لتحديد معدّل الطلبات: phone → [timestamps]
const sendLog = new Map();

// تنظيف دوري للسجل حتى لا ينمو مع الوقت
setInterval(() => {
  const now = Date.now();
  for (const [phone, times] of sendLog) {
    const fresh = times.filter((t) => now - t < 60 * 1000);
    if (fresh.length) sendLog.set(phone, fresh);
...
const sendOtp = async (req, res) => {
  try {
    ...
    if (!canSend(phone)) {
      return res.status(429).json({ success: false, message: 'محاولات كثيرة، انتظر نصف دقيقة ثم أعد المحاولة' });
    }
...
module.exports = wrapAll({ sendOtp, verifyOtp, getMe, logout, normalizePhone });
