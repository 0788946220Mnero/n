...
const notifyCustomers = async ({ title, body, link = '', image = '' }) => {
  ...
  for (let i = 0; i < tokens.length; i += 500) {
    ...
    const message = {
      ...
      data: { link: String(link || ''), kind: 'promo' },
  ...
  console.log(`📣 إشعار الزبائن "${title}": أُرسل ${sent} / فشل ${failed} من أصل ${tokens.length}`);
  return { sent, failed, error: '' };
};

module.exports = { isEnabled, sendToTokens, notifyNewOrder, notifyCustomers };
